...
function create(_a) {
    ...
    return function hydrate(key, store, initialState, customArgs) {
        ...
        var reactionRef = mobx_1.reaction(function () { return serializr_1.serialize(schema, store); }, function (data) { return storage.setItem(key, !jsonify ? data : JSON.stringify(data)); }, {
            delay: debounce
        });
